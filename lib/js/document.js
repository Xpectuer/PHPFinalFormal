...
$.ajax({
    ...
    success:function (dataFeedBack) {
        $.each(dataFeedBack,function (index,obj) {
            ...
            let commit=" <div class=\"eskimo_comment_wrapper\" id=\'comment_"+pesId+"\'>\n" +
                ...
                "                                                        <button class='delete_comment_btn'  ><i class=\"fa fa-tipes\">删除</i></button>      "+
        ...
        $.ajax({
            ...
            success:function (dataFeedBack){
                ...
                $.each(dataFeedBack,function (index,obj) {
                    ...
                    image="<img alt='' src='../lib/uploads/2019/12/img"+imaId+".jpg' />";
                    let pessId=obj["passageId"];
                    let selector="#commentlist> #comment_"+pessId;
        ...
        $("#submit").on("click",function () {
            ...
            if($commentToAdd.length>=10){
                ...
                //let time=new Date();
                //  let nowTime=time.toDateString()+" "+time.toLocaleTipeString();
                //前后台格式不一致，干脆同样改成后台时间
                //ajax
                $.ajax({
                    type:"POST",
                    ...
                    success:function (dataFeedBack) {
                        ...
                        pesId=dataFeedBack["passage_id"];//最新一条评论
                        time=dataFeedBack["created_time"];
                        authorId=dataFeedBack["authorId"];


                        image="<img alt='' src='../lib/uploads/2019/12/img"+imaId+".jpg' />";
                        let commitToAdd=" <div class=\"eskimo_comment_wrapper\" id=\'comment_"+pesId+"\'>\n" +
                            "                                <div class=\"eskimo_comments\">\n" +
                            "                                    <div class=\"eskimo_comment\">\n" +
                            "                                        <div class=\"eskimo_comment_inner\">\n" +
                            "                                            <div class=\"eskimo_comment_left\">\n" +
...
comList.on("click",selector2,function () {
    ...
    let nowTime=time.toDateString()+" "+time.toLocaleTipeString();
    //从前台ID属性中取到评论id
    let $pesId=$(this).parent().parent().attr("id").split("_")[1];
    //提示框
...
comList.on("click",".delete_comment_btn",function () {
    ...
    $.ajax({
        type:"POST",
        url:"services/deletePassage.php",
